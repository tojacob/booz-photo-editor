...
class MerchBase {
  ...
  protected jacketSize: HTMLInputElement;
  protected jacketSizeDescription: HTMLInputElement;
  protected shoesSizeUs: HTMLInputElement;
  protected shoesSizeMx: HTMLInputElement;
  protected type: HTMLSelectElement;
  protected whatsappFilter: HTMLInputElement;
  ...
  constructor(protected formIndex: string) {
    ...
    this.jacketSize = <HTMLInputElement>this.getFormInput(controls.jacketSize);
    this.jacketSizeDescription = <HTMLInputElement>this.getFormInput(controls.jacketSizeDescription);
    ...
    this.whatsappFilter = <HTMLInputElement>this.getFormInput(controls.whatsappFilter);
  }
  ...
  protected setMerchWhatsappFilter(template: HTMLElement): void {
    if (!this.whatsappFilter.checked) return;

    const filter = <HTMLDivElement>template.getElementsByClassName('whatsapp-filter')[0];
...
export class Merch extends MerchBase {
  ...
  private async createImageTypeGeneral(image: File): Promise<ProcessedImage[]> {
    ...
    this.setMerchWhatsappFilter(template);
  ...
  private async createImageTypeSimple(image: File): Promise<ProcessedImage[]> {
    const simpleTemplate = <HTMLElement>document.getElementById('template-simple');
    const template = <HTMLElement>simpleTemplate.cloneNode(true);
    const processedImages: ProcessedImage[] = [];
  ...
  private async createImageTypeJacket(image: File): Promise<ProcessedImage[]> {
    const jacketTemplate = <HTMLElement>document.getElementById('template-jacket');
    const merchSizes = this.getMerchSizes(this.jacketSize.value);
    const sizeName = controls.jacketSize.name;
    const sizeDescName = controls.jacketSizeDescription.name;
    const sizeDescVal = this.jacketSizeDescription.value;
    const processedImages: ProcessedImage[] = [];

    for (const merchSize of merchSizes) {
      const template = <HTMLElement>jacketTemplate.cloneNode(true);
  ...
  public async createImages(): Promise<ProcessedImage[]> {
    ...
    const templates: Record<string, (image: File) => Promise<ProcessedImage[]>> = {
      ...
      [formTypes.jacket]: (image: File) => this.createImageTypeJacket(image),
      [formTypes.pant]: (image: File) => this.createImageTypePant(image),
      [formTypes.shoes]: (image: File) => this.createImageTypeShoes(image),
      [formTypes.simple]: (image: File) => this.createImageTypeSimple(image)
    };
